...
try{
    ...
    if($menu){
        $Interactive=$true
        Write-Host 'QMT IPO 3.4.0 - Offline setup'
        Write-Host '1 New install (disabled)  2 Resume new install'
        Write-Host '3 Local configuration  4 Doctor  5 Open console'
        Write-Host '6 Upgrade  7 Code rollback  8 Health'
        Write-Host '9 Export diagnostics  10 Calendar coverage  11 Recovery plan'
        ...
        $choice=Read-Host 'Select operation';if(-not $map.ContainsKey($choice)){throw 'invalid_menu_choice'}
        $Operation=$map[$choice]
        $Root=Read-Host 'Installation directory (separate from package; existing root for upgrade)'
        if($Operation -in @('Upgrade','Rollback')){
            $ConfirmMaintenance=((Read-Host 'Allow maintenance after backup and closing console? Type YES') -ceq 'YES')
        }
        if($Operation -eq 'Rollback'){$RollbackId=Read-Host 'RollbackId from upgrade receipt'}
        if($Operation -eq 'ExportSupport'){
            $Output=Read-Host 'New diagnostic ZIP path outside installation'
            $ConfirmExport=((Read-Host 'Export sanitized summary locally? Type YES') -ceq 'YES')
        }
    }
    if($Operation -eq 'VerifyPackage'){$result=$checked;$rc=0}
        ...
        if($Operation -eq 'OpenConsole'){
            ...
            Start-Process -FilePath $pythonw -ArgumentList ('-B "'+$panel+'" --root "'+$Root+'"') -WorkingDirectory $Root -WindowStyle Hidden | Out-Null
...
if($Interactive){
    if($result.ok){Write-Host 'Completed. New installs remain disabled; account acceptance is separate.'}
    else{Write-Host 'Incomplete. Preserve configuration, ledger and receipts for diagnosis.'}
